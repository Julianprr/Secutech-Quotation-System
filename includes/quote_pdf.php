...
function generate_quote_pdf(array $company, array $quote, array $items, float $subtotal, float $vat_total, float $grand_total): string
{
    $pdf = new SimplePdf();
    ...
    $pdf->setColor('#ffffff');
    $pdf->textRight(198, 13, 'QUOTATION', 15, true);

    $pdf->setColor($GREEN);
    $pdf->textRight(198, 20, $quote['quote_number'], 10, true);
    ...
    if (!empty($company['registration_number'])) {
        $info_parts[] = 'Reg: ' . $company['registration_number'];
    }
    if (!empty($company['vat_number'])) {
        $info_parts[] = 'VAT: ' . $company['vat_number'];
    }
    if (!empty($company['phone'])) {
        $info_parts[] = 'Tel: ' . $company['phone'];
    }
    if (!empty($company['email'])) {
        $info_parts[] = 'Email: ' . $company['email'];
    }

    $info_line = implode('  |  ', $info_parts);

    $pdf->setColor($GREY);
    $info_lines = $pdf->wrapText($info_line, 186, 7.5);
    ...
    $pdf->text(14 + $col_w + 6, $box_top + 4.3, 'QUOTATION DETAILS', 8, true);

    $pdf->setColor($DARK_TEXT);
    $by = $box_top + 11;

    $pdf->text(14, $by, $quote['company_name'], 9.5, true);
    $by += 5;

    if (!empty($quote['contact_name'])) {
        $pdf->text(14, $by, 'Contact: ' . $quote['contact_name'], 8.5);
        $by += 4.5;
    }
    if (!empty($quote['telephone'])) {
        $pdf->text(14, $by, 'Tel: ' . $quote['telephone'], 8.5);
        $by += 4.5;
    }
    if (!empty($quote['customer_email'])) {
        $pdf->text(14, $by, 'Email: ' . $quote['customer_email'], 8.5);
        $by += 4.5;
    }
    ...
    $pdf->text($dx, $dy, 'Quote Date: ' . $quote['quote_date'], 8.5);
    $dy += 4.5;
    $pdf->text($dx, $dy, 'Valid Until: ' . $quote['valid_until'], 8.5);
    $dy += 4.5;

    if (!empty($quote['sales_person'])) {
        $pdf->text($dx, $dy, 'Sales Person: ' . $quote['sales_person'], 8.5);
        $dy += 4.5;
    }

    if (!empty($quote['payment_terms'])) {
        $terms_lines = $pdf->wrapText('Payment Terms: ' . $quote['payment_terms'], $col_w - 4, 8.5);
        foreach ($terms_lines as $line) {
            $pdf->text($dx, $dy, $line, 8.5);
            $dy += 4;
        }
    }
    ...
    $col_x = [
        'code' => 12, 'desc' => 32, 'qty' => 118, 'unit_price' => 140,
        'disc' => 165, 'vat' => 178, 'total' => 198,
    ];
    ...
    foreach ($items as $idx => $item) {
        ...
        $pdf->text($col_x['desc'] + 1, $y + 4.5, $desc_line, 8);
    ...
    if (!empty($quote['notes'])) {
        ...
        $pdf->setColor('#333333');
        $note_lines = $pdf->wrapText((string) $quote['notes'], 186, 8);
    ...
    if (!empty($company['bank_name']) || !empty($company['account_number'])) {
        ...
        if (!empty($company['account_holder'])) {
            $bank_lines[] = 'Account Holder: ' . $company['account_holder'];
        }
        if (!empty($company['bank_name'])) {
            $bank_lines[] = 'Bank: ' . $company['bank_name'];
        }
        if (!empty($company['account_number'])) {
            $bank_lines[] = 'Account Number: ' . $company['account_number'];
        }
        if (!empty($company['branch_code'])) {
            $bank_lines[] = 'Branch Code: ' . $company['branch_code'];
        }
        if (!empty($company['account_type'])) {
            $bank_lines[] = 'Account Type: ' . $company['account_type'];
        }
        if (!empty($company['swift_code'])) {
            $bank_lines[] = 'SWIFT Code: ' . $company['swift_code'];
        }

        $bank_lines[] = 'Reference: ' . $quote['quote_number'];
